...
namespace PBL3_MVC.Areas.Customer.Controllers
{
    public class BillsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            List<BillModel> bills = db.Seats.Where(s => s.Bill.CustomerID == userSession.AccountID).Select(s => new BillModel { Id = (int)s.BillID, SeatID = s.SeatID, CustomerName = s.Bill.Customer.Name, BusStationName = s.Schedule.Bus.BusStation.Name, BusName = s.Schedule.Bus.BusName, SeatNumber = s.SeatNumber, Departure = s.Schedule.Route.Departure, Destination = s.Schedule.Route.Destination, DepartureTime = s.Schedule.DepartureTime, DestinationTime = s.Schedule.DestinationTime, Price = s.Price, OrderDate = s.Bill.OrderDate }).ToList();
